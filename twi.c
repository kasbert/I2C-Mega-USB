/*
  ...
  Modified 2022 by Jarkko Sonninen to replace timeout and Arduino dependencies
*/
...
{
  uint8_t val = ((F_CPU / frequency) - 16) / 2;
  if (val < 10) {
    val = 10;
  }
  TWBR = val;
...
static void timeout_set(uint16_t t) {
  if (t <= 1000) {
    // Set timer0 counter
    TCNT0 = 255 - t / 4; // 16MHz / 64 ~ 4us/tick
    ...
    TCNT0 = 255 - t / 16; // 16MHz / 256 ~ 16us/tick
    // Enable interrupt for timer0 overflow
    //TIMSK0 = (1 << TOIE0);
    TIFR0 |= (1 << TOV0);
    // set  prescaler to 256 and start the timer
    TCCR0B = (0<<CS00)|(01<<CS01)|(1<<CS02);
  }
}

static inline uint8_t is_timeout(void) {
  return (TIFR0 & (1 << TOV0));
}
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
  ...
  timeout_set(twi_timeout_us);
  while(TWI_READY != twi_state){
    if(twi_timeout_us && is_timeout()) {
